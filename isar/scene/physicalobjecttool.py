...
def draw_physical_object_image(opencv_img, phys_obj: PhysicalObject):
    ...
    image_frame = ImageFrame(opencv_img.shape[1], opencv_img.shape[0])
    x, y = util.relative_coordinates_to_image_coordinates(image_frame, rel_x, rel_y)
...
def draw_physical_object_bounding_box(opencv_img, phys_obj: PhysicalObject):
    ...
    image_frame = ImageFrame(opencv_img.shape[1], opencv_img.shape[0])
    tl = util.relative_coordinates_to_image_coordinates(image_frame, *phys_obj.top_left)
    br = util.relative_coordinates_to_image_coordinates(image_frame, *phys_obj.bottom_right)
